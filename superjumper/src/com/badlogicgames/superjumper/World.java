...
public class World {
	...
	public static final Vector2 gravity = new Vector2(0, -15);
	...
	public void generateLevel (int level) {
		...
		float worldHeight = WORLD_HEIGHT - WORLD_WIDTH / 2;
		float levelPara = (float)level/GameScreen.LEVEL_MAX;
		Log.d(TAG,"levelPara is " + levelPara);
		worldHeight = worldHeight*levelPara;
		while (y < worldHeight) {
			//[random the platforms' position , notice that the (x,y) of platfom is its center position.]
			//level is higher moving platform is more.
			int type = rand.nextFloat() < levelPara ? Platform.PLATFORM_TYPE_MOVING : Platform.PLATFORM_TYPE_STATIC;
			float x = rand.nextFloat() * (WORLD_WIDTH - Platform.PLATFORM_WIDTH) + Platform.PLATFORM_WIDTH / 2;

			Platform platform = new Platform(type, x, y);
			if(type == Platform.PLATFORM_TYPE_MOVING){
				float v = Platform.PLATFORM_VELOCITY*5*levelPara;
				platform.setVelocity(v);
			}
			...
			float minus = (maxJumpHeight*(1-levelPara));
			Log.d(TAG, "minus is " + minus);
			Log.d(TAG, "maxJumpHeight is " + maxJumpHeight);
			y -= minus;
		}
...
//					}
	...
	private void checkCastleCollisions () {
		if (OverlapTester.overlapRectangles(castle.bounds, bob.bounds) ||bob.position.y > castle.position.y) {
